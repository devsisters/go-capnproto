...
func (n *node) defineField(w io.Writer, f Field) {
	...
	case TYPE_TEXT:
		...
		// note that the bytes would have the \x00 byte at the end of
		// the string included in the []byte, so we use ToDataTrimLastByte()
		// instead of ToData() to avoid that.
		fprintf(&g, "func (s %s) %sBytes() []byte { return C.Struct(s).GetObject(%d).ToDataTrimLastByte() }\n", n.name, fname, off)
...
func init() {
	...
	if disable, err := strconv.ParseBool(os.Getenv("GO_CAPNP_CAPLIT_MARSHAL_DISABLE")); err == nil {
		disabledCaplitMarshal = disable
	}
	if disable, err := strconv.ParseBool(os.Getenv("GO_CAPNP_CAPLIT_UNMARSHAL_DISABLE")); err == nil {
		disabledCaplitUnarshal = disable
	}
	if disable, err := strconv.ParseBool(os.Getenv("GO_CAPNP_UTIL_DISABLE")); err == nil {
		disabledUtil = disable
	}
	...
	for _, reqf := range req.RequestedFiles().ToArray() {
		...
		for _, n := range f.nodes {
			...
			case NODE_ENUM:
				...
				if !disabledCaplitMarshal {
					n.defineTypeCaplitMarshalFuncs(&buf)
				}
				if !disabledCaplitUnarshal {
					n.defineTypeCaplitUnmarshalFuncs(&buf)
				}
			case NODE_STRUCT:
				if !n.Struct().IsGroup() {
					if !disabledBase {
		...
		assert(err == nil, "%v\n", err)
		fprintf(file, "// Code generated by %s - DO NOT EDIT.\n\n", go_capnproto_import)
